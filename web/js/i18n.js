...
(function (global) {
  ...
  var T = {
    zh: {
      ...
      'settings.fishBaseHint': '两套接口都在用：留空=旧版 Open API（fishaudio.org/api/open/v1，支持本地样本自动复刻）；填 https://api.fish.audio 则走新版（/v1/tts + model 头），此路需先在 fish.audio 里建好音色并把 id 填到下面。',
      'settings.fishModel': 'Fish 引擎模型',
      'settings.fishModel.hint': '引擎 ID，和音色 ID 不是同一个字段。旧版默认 fishaudio-s21pro-flash；新版（api.fish.audio）默认 s2.1-pro-free。',
      'settings.fishVoice': 'Fish 音色 ID',
      'settings.fishVoice.hint': '留空则用本地莱莎序章原声自动复刻（仅旧版 Open API 支持）；新版 api.fish.audio 必须填你在 fish.audio 建好的音色 id。',
    ...
    ja: {
      ...
      'settings.fishBaseHint': '二つの口が混在しています：空欄＝旧 Open API（fishaudio.org/api/open/v1、ローカル音声の自動クローン可）；https://api.fish.audio を入れると新 API（/v1/tts + model ヘッダ）で、先に fish.audio で音色を作って下に id を入れる必要があります。',
      'settings.fishModel': 'Fish エンジンモデル',
      'settings.fishModel.hint': 'エンジン ID。音色 ID とは別欄です。旧 API の初期値は fishaudio-s21pro-flash、新 API（api.fish.audio）は s2.1-pro-free。',
      'settings.fishVoice': 'Fish 音色 ID',
      'settings.fishVoice.hint': '空欄ならローカルのライザ序章音声で自動クローン（旧 Open API のみ）。api.fish.audio を使う場合は fish.audio で作った音色 id を必ず入れてください。',
    ...
    en: {
      ...
      'settings.fishBaseHint': 'Two surfaces are in the wild: empty = the older Open API (fishaudio.org/api/open/v1, auto-clones from local samples); paste https://api.fish.audio for the current one (/v1/tts + a model header), which needs a voice id you created on fish.audio.',
      'settings.fishModel': 'Fish engine model',
      'settings.fishModel.hint': 'Engine id, separate from the voice id. Older API defaults to fishaudio-s21pro-flash; the current one (api.fish.audio) to s2.1-pro-free.',
      'settings.fishVoice': 'Fish voice ID',
      'settings.fishVoice.hint': 'Leave empty to auto-clone from local Ryza prologue audio (older Open API only). On api.fish.audio you must paste a voice id created on fish.audio.',
  ...
  T.id = inherit(T.en, {
    ...
    'skin.postureHint': 'Duduk/berdiri mengikuti lokasi; ganti di bar atas yang punya keduanya',
    'posture.sit': 'Duduk',
    ...
    'settings.fishBaseHint': 'Ada dua permukaan: kosong = Open API lama (fishaudio.org/api/open/v1, kloning otomatis dari sampel lokal); tempel https://api.fish.audio untuk yang terbaru (/v1/tts + header model), perlu id suara yang dibuat di fish.audio.',
    'settings.fishModel': 'Model engine Fish',
    'settings.fishModel.hint': 'Id engine, terpisah dari id suara. API lama default fishaudio-s21pro-flash; yang terbaru (api.fish.audio) s2.1-pro-free.',
    'settings.fishVoice': 'ID suara Fish',
    'settings.fishVoice.hint': 'Kosongkan untuk kloning otomatis dari audio prolog Ryza lokal (hanya Open API lama). Di api.fish.audio, tempel id suara yang dibuat di fish.audio.',
